export const lengthDescriptions = [
  'Short: A brief summary, focusing on the core message with minimal words.',
  'Long: A comprehensive summary that delves deeper into the topic.',
  'Auto: Automatically adjusts the length based on the content and context for optimal summarization (recommended).'
];

export const toneDescriptions = [
  'Casual: Relaxed and conversational, ideal for general reading.',
  'Informative: Focused on delivering clear facts and explanations.',
];

export const formatDescriptions = [
  'Paragraphs: A flowing narrative with multiple paragraphs',
  'Bullet Points: Structured using bullet points.',
  'Mix: A combination of paragraphs and bullet points for clarity.',
  'Q&A: Information presented in a question-and-answer format.',
];

export const readingLevelDescriptions = [
  'Simple: Easy-to-read language, suitable for younger audiences or non-native speakers.',
  'Standard: A balanced reading level for general audiences.',
  'Advanced: More complex vocabulary and sentence structures for expert readers.'
];

export const languageDescriptions = [
  'English: A widely spoken global language.',
  'Spanish: Spoken across Spain, Latin America, and the U.S.',
  'French: Common in France, Canada, and parts of Africa.',
  'Arabic: Spoken in the Middle East and North Africa.',
  'Chinese (Simplified): Used in mainland China and Singapore.',
  'Chinese (Traditional): Used in Taiwan, Hong Kong, and Macau.',
  'Hindi: One of the most spoken languages in India and globally.',
  'Japanese: Primarily spoken in Japan.',
  'Russian: Spoken in Russia, Eastern Europe, and Central Asia.',
  'Portuguese: Used in Portugal, Brazil, and some African countries.',
  'German: Spoken in Germany, Austria, and Switzerland.',
  'Italian: Spoken in Italy and known for its cultural contributions.',
  'Korean: Spoken in South and North Korea.',
  'Turkish: Spoken in Turkey and Cyprus.',
  'Bengali: Common in Bangladesh and parts of India.',
  'Vietnamese: The main language of Vietnam.',
  'Dutch: Spoken in the Netherlands and Belgium.',
  'Greek: Spoken in Greece and Cyprus.',
  'Hebrew: The official language of Israel.',
  'Polish: Spoken in Poland and worldwide.',
  'Swedish: Spoken in Sweden and Finland.',
  'Ukrainian: The official language of Ukraine.',
  'Hungarian: Spoken in Hungary.',
  'Igbo: Spoken in southeastern Nigeria.'
];
